...
if __name__ == "__main__":
    ...
    #get dataset stat
    dataset_stat = get_dataset_stat(dataset=args.dataset)
    print(f'Total number of datasets: {len(dataset_stat.keys())}')
    limit = 2
    valid_users = []
    for user in dataset_stat:
        if len(dataset_stat[user].keys()) >= limit:
    ...
    temp=[]
    for user in clients_4_training:
        if user not in valid_users:
            continue
        temp.append(user)
    clients_4_training = temp
    ...
    for round in track(
        ...
        for user_id in selected_learners:
            ...
            serialized_model_params, training_stats = participants[user_id].train(
                global_model=global_model,
                hessian_free=args.hf,
                eval_while_training=args.eval_while_training, training_stats=training_stats,
            )
        ...
        for learner_id in model_params_cache.keys():            
            ...
            for worker_id in selected_task_pers:
                ...
                personalized_stats = participants[worker_id].pers_params(local_model= local_model, worker_epochs= args.worker_epochs, training_stats=training_stats)
